...
- 프로젝트 저장/열기(.mmcad): 불러오기로 setGeo/wind/mat 하면 기존 무효화 effect
  (`useEffect(()=>setFemmCal(null),[geo,wind,mat])`)가 파일의 femmCal을 덮어 지운다 →
  ref 1회 스킵 가드(skipFemmReset)로 보존. setState는 배치되어 effect가 1회만 돌므로 1회 스킵 충분.
  복원은 `{...DEFAULT, ...loaded}` 오버레이로 누락키(버전차) 방어, results는 항상 재계산(스냅샷은 기록용).
  교훈: "X 바뀌면 Y 리셋" 자동무효화 effect가 있는 상태를 파일에서 복원할 땐 그 effect와 경쟁한다 — 로드 경로용 스킵.
