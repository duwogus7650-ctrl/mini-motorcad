...
# 7차: AC 동손 + 효율맵 마감 (2026-06-13)

- [x] DXF: 회전 자동정렬 제거(치수·슬롯/극수만 추출), 불러오기는 표시 중심정렬만
- [x] 효율맵: 피크전류(1.6×)로 토크축 확장(효율섬 닫힘), iso-효율 등고선(marching-squares)+라벨,
      포락선 다각형 클리핑(경계 계단 제거)
- [x] AC 동손(근접효과) 모델: Pyrhönen 환산높이+층수, fe² 스케일, cAC 보정계수(기본1.0)
      → 정격 R_ac/R_dc 1.018(+0.58W, 효율 영향 거의 없음), 효율맵 고속 반영
- [x] node 검증(정격 1.018 / 2× 1.072 / 3× 1.161), build 통과
- [ ] kro80_v3 FEA 효율맵 나오면 cAC·철손계수 캘리브레이션 (사용자 FEA 진행 중)

## 7차 비고

- 효율맵 모습은 Motor-CAD에 맞춤(jet+등고선+섬). 절대 효율섬 크기는 FEA가 정확(해석식 한계).
- AC 동손은 표시 속도범위(~4200rpm=1.3×fe)에서 ~3%까지라 영향 modest — cAC로 FEA에 맞춤 예정.
