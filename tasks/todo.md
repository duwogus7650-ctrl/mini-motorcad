...
- [x] THERM0 + Thermal 탭: 3노드 LPTN(권선→철심→하우징→공기), 정상상태
- [x] 열저항: R_slot(라이너+함침/접촉면), R_yoke(계면접촉+백아이언), R_conv(1/hA, 하우징표면)
- [x] 입력: 냉각방식(자연10/강제60/전도200)·하우징 외경·길이·접촉계수·재질 k, 주위온도
- [x] 열-전기 연성: 권선온도→ρ(T)→동손 반복 수렴. 권선 포화온도 출력 + 노드/저항 분해
- [x] 포화온도-속도 곡선(손실 속도외삽). 과열 경고(>130°C), 입력Tcu 불일치 안내
- [x] node 검증: 자연대류 196°C(과열) / 강제 59 / 전도 44, 하우징 확대 효과 정상. build 통과

## 8차 비고

- 자연대류+텍스트북 기본값(사용자 선택). h·접촉저항이 결과 지배 — 열 측정/FEA로 보정 대상.
- 1250W-jk는 손실 77W라 자연대류 연속정격 불가(196°C) — 강제/전도 냉각 필요(물리적으로 타당).
- 열 기준데이터 없어 절대값 미보정 — 경향·하우징/냉각 영향 파악용.
...
# 슬롯단면/권선/전류밀도/보정계수 4건 (2026-06-22, 사용자 피드백)

- [x] ① 직선바닥+두꺼운 디바이더 바닥관통 — 디바이더 깊은끝을 바닥윤곽 5각형으로 클립(직선/호 공통)
- [x] ② 권선 바닥기준 패킹 — 깊이순 정렬로 바닥부터 채우고 부분충전 시 개구쪽이 비도록
- [x] ③ 전류밀도 9.470 vs MC 8.65 — 식 검증완료(나동선기준), 동선경 입력 1.05→1.10 정정으로 일치(코드 무변경)
- [x] ④ 보정계수 자동산정 — FEMM이 형상에서 자동대체함을 명확화, 보정계수 박스에 3상태 안내/경고 추가

## Review

- 검증: vite build ✓ / ESLint 0 ✓ / tools/verify_divider_clip.mjs PASS(4형상×2바닥×디바이더~10mm 비관통·좌표유한).
- 적대 리뷰: 워크플로 리그레션 리뷰어 깨끗(코드변경 불필요 nit 1건=깊이정렬이 over-full에도 적용, 물리적으로 옳음).
  기하/물리·UI 리뷰어는 서브에이전트 API 일시불통(소켓/타임아웃)으로 죽어 메인루프에서 직접 인라인 재검증.
- ②는 슬롯이 꽉 차면 시각변화 없음(정상); 턴수↓·슬롯↑로 슬랙 생기면 개구쪽 빈공간으로 확인 가능.
- ③ wireDia는 피복외경(>1.10)으로 둬야 cuSlotFill/wireSlotFill 점적률 분리가 맞음.

# 프로젝트 저장/열기 (.mmcad) (2026-06-22, 사용자 요청)

- [x] 헤더에 💾 프로젝트 저장 / 📂 프로젝트 열기 추가
- [x] 저장: 형상·권선·재질·운전점·열·femmCal + 결과 스냅샷을 .mmcad(JSON)로
- [x] 열기: 입력 전체 복원(기본값 오버레이로 누락키 방어) + 결과 재계산, femmCal 보존(skipFemmReset 1회 가드)
- [x] 검증: build/ESLint + tools/verify_project_io.mjs 10/10 (라운드트립·부분파일·femmCal·잘못된파일거부)

## Review

- results는 불러올 때 항상 재계산(스냅샷은 기록용) → 오래된 파일도 최신 엔진 반영.
- 런타임 확인 권장: FEMM 보정 적용상태 저장→열기 시 헤더 배지 유지(skipFemmReset는 effect 순서 추론으로 검증).
